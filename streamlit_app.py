...
with col2:
    ...
    st.metric("Loaded Resumes", 0, label_visibility="collapsed")

# Process uploaded file or use sample
if uploaded_file:
...
# Sort by final score
ranked_resumes = resume_df.sort_values(by='final_score', ascending=False)
...
st.text_area(
    "Raw Resume:",
    value=str(selected_candidate[resume_column])[:2000] + "..." if len(str(selected_candidate[resume_column])) > 2000 else selected_candidate[resume_column],
    height=200,
    disabled=True
)
